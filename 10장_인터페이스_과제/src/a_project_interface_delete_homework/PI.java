...
public class PI implements P{ //PI : 고객 클래스를 관리하는 '매니저 클래스'(이 클래스만 외부에 알리면 됨)
	...
	public void search() { 	//외부에 노출된 메서드(주민등록번호를 입력받아 고객의 정보를 출력)
		...
		if(storedPersonNum == 0) {
			System.out.println("등록된 고객정보가 없어 검색을 진행할 수 없습니다.");
			return;
		}
	...
	private String search(String serialNumber) {	//사용자가 입력한 주민등록번호
		...
		for(Person person:p) { //person:배열에 저장된 값(=고객객체의 주소)
			...
			if(person == null) {		//향상된 for문 사용 시, 배열에 저장할 공간이 남은 상태로 null값을 만난 경우 예외발생 방지
	...
	public void delete() { // ★★[08/12 과제]삭제 메서드 구현 - 향상된 for문 사용 불가. index필요
		if(storedPersonNum == 0) { //저장한 고객정보가 없는 경우
			System.out.println("등록된 고객정보가 없어 삭제할 수 없습니다.");
			return;
		}
		
		//삭제된 공간을 메우기 위해 뒤의 고객객체들을 앞으로 당김 (주의:삭제 후 storedPersonNum 감소해야 함)
		//delete() 순서-1 : 주민등록번호 입력
		System.out.print("삭제할 고객의 주민등록번호를 입력하세요 > ");
		String serialNumber = MenuViewer.sc.nextLine().trim();
		
		//delete() 순서-2 : private 리턴타입 delete(주민등록번호) - 실질적인 삭제 작업을 담당하는 메서드
		String personName = delete(serialNumber);
		
		//delete() 순서-3 : 리턴값을 받아서 찾는 고객이 있으면 "삭제완료"
		//									  없으면 "삭제안됨" 출력
		if(personName != null) {
			System.out.println("'" + personName + "'님의 고객정보를 삭제했습니다.");
		}else System.out.println("고객을 찾을 수 없어 삭제하지 못했습니다.");
		
	}
	
	private String delete(String serialNumber) { //실질적인 삭제 작업을 담당하는 메서드
		
		String personName = null; //삭제되는 고객명
		
		for(int i = 0; i < storedPersonNum; i++) {		//고객이 저장된 인덱스까지 반복
			if(p[i].getSerialNumber().equals(serialNumber)) {
				personName = p[i].getName(); //일치하는 주민번호의 고객명을 담음
				
				if(i == storedPersonNum-1) { //저장된 마지막 인덱스 값을 삭제할 경우
					p[i] = null; 		//null 대입(고객객체주소 삭제)
				}else {
					while(i < storedPersonNum-1) { 	//마지막 인덱스가 아닌 경우 삭제된 자리 뒤의 객체들을 앞으로 당김
						p[i] = p[i + 1];
						i++;
					}
				...
				storedPersonNum--;	//저장된 고객정보 길이 감소
				return personName; 	//삭제 성공
			}//주민등록번호 if 문 끝
		}//for 문 끝
		
		return personName;	//삭제 실패
	}
